...
async function fetchPunchRowsByKnownShapes(periodId) {
  ...
  if (periodId) {
    modernQuery = modernQuery.eq('payroll_period_id', periodId);
  }

  const modern = await modernQuery;
  if (!modern.error) return modern;

  const missingPeriodId = isMissingColumnError(modern.error, 'payroll_period_id');
  const missingPunchAt = isMissingColumnError(modern.error, 'punch_at');
  if (!missingPeriodId && !missingPunchAt) {
    return modern;
  }

  // Legacy fallback for environments where dtr_punches still uses date/time columns.
