...
const SUPABASE_URL = window.SUPABASE_URL || "https://qzkzugzfpegozpiqutdv.supabaseClient.co"
...
const supabaseClient = createClient(SUPABASE_URL, SUPABASE_KEY, {
  auth: {
    // We use anon access + RLS policies; no interactive auth needed.
    // Disabling session persistence avoids Navigator Lock timeouts on sb-*-auth-token.
...
function __kvSetupRealtimeChannel(force = false) {
  try {
    ...
    const channel = supabaseClient.channel('kv_store_sync');
...
async function __kvHydrateOnce(){
  try {
    ...
    for (const row of (data || [])) {
      ...
      if (preferLocal) {
        try { await supabaseClient.from(TABLE).upsert({ key: k, value: JSON.parse(cur) }, { onConflict: 'key' }); } catch(_){}
        continue;
      }
...
(function(){
  document.addEventListener('DOMContentLoaded', function(){
    ...
    async function listBackups(){
      if(!supabase || !supabaseClient.storage){ setStatus('Supabase storage unavailable', true); return; }
      try {
    ...
    async function restoreFromCloud(){
      ...
      try {
        ...
        const { data, error } = await supabaseClient.storage.from(BUCKET).download(name);
        if (error) throw error;
    ...
    async function backupNow(){
      try {
        ...
        try {
          if (supabase && supabaseClient.storage){
            const { error: upErr } = await supabaseClient.storage.from(BUCKET).upload(filename, blob, { upsert: true, contentType: 'application/json' });
    ...
    async function healthCheck(){
      ...
      try {
        const r = await supabaseClient.from(KV_TABLE).select('key').limit(1);
        if (!r.error){ results.kv = true; logMsg('KV table OK'); } else { logMsg('KV issue: ' + r.error.message); }
      } catch(e){ logMsg('KV exception: ' + e.message); }
      try {
        const r2 = await supabaseClient.from(DTR_TABLE).select('count', { count: 'exact', head: true });
      ...
      try {
        if (supabase && supabaseClient.storage){
          const r3 = await supabaseClient.storage.from(BUCKET).list('', { limit: 1 });
