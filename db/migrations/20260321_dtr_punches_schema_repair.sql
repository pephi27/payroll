...
BEGIN
  ...
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS id text';
  EXECUTE format('ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS payroll_period_id %s', v_period_id_type);
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS employee_id text';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS project_id text';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS punch_at timestamptz';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS meta jsonb NOT NULL DEFAULT ''{}''::jsonb';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS emp_id text';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS date date';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS time text';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS source text';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS data jsonb';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS updated_by text';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now()';
  EXECUTE 'ALTER TABLE public.dtr_punches ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now()';

  select format_type(a.atttypid, a.atttypmod)
    into v_current_period_id_type
  from pg_attribute a
  where a.attrelid = 'public.dtr_punches'::regclass
    and a.attname = 'payroll_period_id'
    and a.attnum > 0
    and not a.attisdropped;

  if v_current_period_id_type is distinct from v_period_id_type then
    execute format(
      'alter table public.dtr_punches alter column payroll_period_id type %1$s using nullif(btrim(coalesce(payroll_period_id::text, '''''')), '''')::%1$s',
      v_period_id_type
    );
  end if;
END;
...
SET payroll_period_id = COALESCE(
      NULLIF(btrim(COALESCE(payroll_period_id::text, '')), ''),
...
WHERE punch_at IS NOT NULL
  ...
  AND COALESCE(btrim(COALESCE(payroll_period_id::text, '')), '') = '';
...
SET meta = jsonb_strip_nulls(
      COALESCE(meta, '{}'::jsonb)
      || jsonb_build_object(
        'payroll_period_id', payroll_period_id::text,
        'empId', employee_id,
...
BEGIN
  IF EXISTS (
    ...
    WHERE id IS NULL OR btrim(id) = ''
       OR payroll_period_id IS NULL OR btrim(payroll_period_id::text) = ''
