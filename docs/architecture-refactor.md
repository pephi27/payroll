...
1. **Prevent duplicate GoTrue clients**
   - `src/config/supabaseClient.js` no longer creates a second Supabase client.
   - It reads the already-initialized legacy singleton from `window.supabase` to avoid shared auth storage conflicts.
2. **Credential source compatibility**
   - The modular layer now depends on the existing legacy Supabase bootstrap key path (`window.SUPABASE_KEY`) instead of creating a parallel client with different key timing.
   - This removes the `Invalid API key` path caused by race/placeholder configuration.
...
- ✅ Manual/delete DTR edit handlers now persist via Supabase sync helper instead of writing LS_RECORDS directly.
