...
11. **Period switch failure restores previous period**
   - Start on Period A and force read-model load failure for Period B (network/RPC failure).
   - Switch to Period B.
   - Expected: Period A is restored and a visible period-switch error message is shown.

12. **Initial period load failure with no previous period**
   - Start app where first period load fails and no prior period is active.
   - Expected: app remains in safe idle state (no stale rows) and shows a visible period-switch failure message.

13. **Selector paths use controlled switch flow**
   - Change period using `activePayrollSelect`, `bpActivePayrollSelect`, and the refresh/apply control in active-week bar.
   - Expected: all routes go through `switchPayrollPeriod`; none bypass with raw period state mutation.

14. **One switch per selector action**
   - Change the active period once from each selector path and observe logs/network/UI state.
   - Expected: only one controlled switch happens per user action.
